...
This document presents the full compliance audit findings and test execution results for all 10 Terraform modules across the repository (including updates merged from `trace-scope-feature`), evaluated under **strict `deny` policy enforcement** against [tests/requirement.md](tests/requirement.md).
...
| [sql-alert-module](sql-alert-module) | `google_project_service`, `google_monitoring_notification_channel`, `google_monitoring_alert_policy` | `@CR.ARCHITECTURE.001`, `@CR.SECURITY.002` | Denies non-GA monitoring services or disabled alert policies | **COMPLIANT** | **PASS** | None. `monitoring.googleapis.com` enabled with valid alert conditions. |
...
| [terraform-log-bucket-cmek](terraform-log-bucket-cmek) | `google_kms_key_ring`, `google_kms_crypto_key`, `google_logging_project_bucket_config` | `@CR.SECURITY.009`, `@CR.SECURITY.034` | Denies missing CMEK settings, missing rotation, or wildcard KMS IAM | **COMPLIANT** | **PASS** | None. Dedicated CMEK logging bucket with 90-day KMS key rotation and CMEK settings. |
...
| [Trace_scope](Trace_scope) | Calls module `./modules/tf_for_scope` | `@CR.ARCHITECTURE.001`, `@CR.SECURITY.002` | Denies trace scope creation failure | **COMPLIANT** | **PASS** | None. Submodule invocation configured with required variables (`monitored_projects`). |
